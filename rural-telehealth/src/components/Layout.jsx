...
import { Home, Stethoscope, FileText, Pill, Menu, X } from 'lucide-react';
import ConnectionBanner from './ui/ConnectionBanner';

const navItems = [
  { path: '/', icon: Home, label: 'Home' },
  { path: '/symptoms', icon: Stethoscope, label: 'Check' },
    ...
    <div className="flex h-screen w-full bg-slate-50 flex-col md:flex-row overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200">
        <div className="p-6 bg-primary text-primary-foreground">
          ...
          <p className="text-sm opacity-90 mt-1">Rural Telemedicine</p>
        </div>
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden h-full">
        ...
        <header className="md:hidden flex items-center justify-between bg-primary text-primary-foreground p-4 shadow-sm z-50">
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 -mr-2 text-white/90 focus:outline-none">
            {isMobileMenuOpen ? <X size={28} /> : <Menu size={28} />}
          </button>
        </header>

        {/* Mobile Connection Banner (top) */}
